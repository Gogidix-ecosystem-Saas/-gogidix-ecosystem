...
import
    com.gogidix.centralconfiguration.dbmigrations.application.service.MigrationSer
    vice;
...
@Slf4j
@Tag(
        name = "Database Migrations", description = "Database
...
public final class MigrationController {
    ...
    @PostMapping("/execute")
    @Operation(
        summary = "Execute database migration", description =
        "Execute migration for specified database")
    @ApiResponse(
        responseCode = "200", description = "Migration executed
        successfully")
    @ApiResponse(
        responseCode = "400", description = "Invalid migration
        request")
    ...
    @PostMapping("/dry-run")
    @Operation(
        summary = "Dry run migration", description = "Simulate
        migration without executing")
    @ApiResponse(
        responseCode = "200", description = "Dry run completed
        successfully")
    @PreAuthorize("hasRole('ADMIN') or hasRole('DBA')")
    ...
    @Operation(
        summary = "Get migration status", description = "Get
        current migration status for database")
    @ApiResponse(
        responseCode = "200", description = "Migration status
    ...
    public ResponseEntity<MigrationStatusResponse> getMigrationStatus(
            @Parameter(
        description = "Database name") @PathVariable String
        databaseName) {
    ...
    @GetMapping("/history")
    @Operation(
        summary = "Get migration history", description = "Get
        paginated migration history")
    @ApiResponse(
        responseCode = "200", description = "Migration history
        retrieved successfully")
    @PreAuthorize("hasRole('ADMIN') or hasRole('DBA') or hasRole('DEVELOPER')")
    ...
    @Operation(
        summary = "Rollback migration", description = "Rollback to
        specific migration version")
    @ApiResponse(
        responseCode = "200", description = "Rollback completed
        successfully")
    ...
    @Operation(
        summary = "Validate migration", description = "Validate
        migration scripts and database state")
    @ApiResponse(
        responseCode = "200", description = "Validation completed
        successfully")
    @PreAuthorize("hasRole('ADMIN') or hasRole('DBA')")
    public ResponseEntity<MigrationStatusResponse> validateMigration(
            @Parameter(
        description = "Database name") @RequestParam String
        databaseName) {
